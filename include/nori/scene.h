...
public:
    ...
    // Emitter *getRandomEmitter(Sampler *sampler) const {
    //     return m_emitters[m_emitters.size() * sampler->next1D()];
    // }

    float getEmiterPdf() const {
...
private:
    ...
    std::vector<EmitterQueryRecord> p_map;
};
