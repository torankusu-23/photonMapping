...
struct PhotonData {
    uint8_t rgbe[4];        //!< Photon power stored in Greg Ward's RGBE format ----如果不需要第四个通道，改成Color3f
    uint8_t theta;            //!< Discretized photon direction (\a theta component)----光子方向用极坐标表示，为什么要这样
     ...
     */
    Vector3f getDirection() const {//又要返回方向？
        return Vector3f(
    ...
    Color3f getPower() const {//znm？
        return Color3f(rgbe[0], rgbe[1], rgbe[2]) * m_expTable[rgbe[3]];
    }
protected:
    // ======================================================================
    /// @{ \name Precomputed lookup tables 奇妙的数据结构，通过预计加速
    // ======================================================================
